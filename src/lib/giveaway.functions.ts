...
const requiredHttpUrl = (maxLength: number) =>
  z.string().trim().max(maxLength).refine(isHttpUrl, {
    message: "Use a complete HTTP or HTTPS link, such as https://instagram.com/your-page",
  });

const optionalHttpUrl = (maxLength: number) =>
  requiredHttpUrl(maxLength).optional().or(z.literal(""));
...
export const getPublicGiveaway = createServerFn({ method: "GET" })
...
export const listPublicWinners = createServerFn({ method: "GET" })
  .inputValidator((input) =>
    z.object({ giveawayId: z.string().uuid().optional() }).parse(input ?? {}),
  )
  .handler(async ({ data }) => {
...
export const submitGiveawayEntry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const ipHash = firstIp
      ? Array.from(
          new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(firstIp))),
        )
...
export const adminListGiveaways = createServerFn({ method: "GET" })
...
export const adminListParticipants = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => z.object({ giveawayId: z.string().uuid() }).parse(input))
...
export const adminUpdateParticipantInstagramLink = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) =>
    z
      .object({
...
export const adminSaveGiveaway = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => giveawayInput.parse(input))
  .handler(async ({ data, context }) => {
...
export const adminDeleteGiveaway = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => z.object({ giveawayId: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
...
export const adminPickWinners = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => z.object({ giveawayId: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
