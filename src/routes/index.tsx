...
import { ArrowUpRight, CalendarDays, ChevronRight, Clock3, Facebook, Instagram, Menu, Send, Sparkles, Trophy, Users, X, Youtube } from "lucide-react";
...
function Home() {
  ...
  const active = useMemo(() => giveaways.filter((giveaway) => giveaway.status === "active" && new Date(giveaway.start_date ?? 0).getTime() <= currentTime && new Date(giveaway.end_date ?? 0).getTime() > currentTime), [currentTime, giveaways]);
  const upcoming = useMemo(() => giveaways.filter((giveaway) => giveaway.status === "active" && new Date(giveaway.start_date ?? 0).getTime() > currentTime).sort((a, b) => new Date(a.start_date ?? 0).getTime() - new Date(b.start_date ?? 0).getTime()), [currentTime, giveaways]);
  const completed = useMemo(() => giveaways.filter((giveaway) => giveaway.status !== "active"), [giveaways]);
  const featured = active[0];

  return <main className="site-shell min-h-screen overflow-hidden"><header className="site-header mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8"><Link to="/" className="brand-lockup"><span className="brand-mark">TT</span><span>Telugu Toon World</span></Link><nav className="hidden items-center gap-7 text-sm font-bold md:flex"><a href="#giveaways" className="text-muted-foreground hover:text-foreground">Giveaways</a><a href="#upcoming" className="text-muted-foreground hover:text-foreground">Upcoming</a><Link to="/past-winners" className="text-muted-foreground hover:text-foreground">Past winners</Link></nav><Button className="hidden md:inline-flex" asChild><a href="https://www.instagram.com/" target="_blank" rel="noreferrer"><Instagram /> Follow on Instagram</a></Button><Button variant="ghost" size="icon" className="md:hidden" onClick={() => setMobileMenu(!mobileMenu)} aria-label={mobileMenu ? "Close menu" : "Open menu"}>{mobileMenu ? <X /> : <Menu />}</Button></header>{mobileMenu && <nav className="mx-5 flex flex-col gap-4 border-b border-border pb-5 text-sm font-bold md:hidden"><a href="#giveaways" onClick={() => setMobileMenu(false)}>Giveaways</a><a href="#upcoming" onClick={() => setMobileMenu(false)}>Upcoming</a><Link to="/past-winners">Past winners</Link></nav>}<section className="hero-band mx-auto max-w-7xl px-5 pt-5 lg:px-8 lg:pt-8"><div className="hero-art"><img src={heroImage} alt="Cheerful cartoon characters celebrating a giveaway" width={1440} height={960} fetchPriority="high" /><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-dot" /> Community giveaways</div><h1 className="hero-title">Win a little <span className="text-brand-coral">toon magic.</span></h1><p className="hero-subtitle">Fun prizes, fair draws, and big smiles from your favorite Telugu cartoon world.</p><div className="mt-7 flex flex-wrap gap-3"><Button size="lg" asChild><a href="#giveaways">Explore giveaways <ArrowUpRight /></a></Button><Button size="lg" variant="outline" asChild><Link to="/past-winners">See past winners <Trophy /></Link></Button></div></div></div></section><section id="giveaways" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><div className="flex flex-wrap items-end justify-between gap-6"><div><div className="eyebrow"><Sparkles className="size-4" /> Enter a giveaway</div><h2 className="section-title mt-4">Enter today’s giveaway.</h2><p className="mt-3 max-w-xl text-muted-foreground">The admin posts the live giveaway here. Enter before the clock runs out; winners are selected in the private studio.</p></div><span className="rounded-full bg-brand-teal/10 px-4 py-2 text-sm font-black text-brand-teal">{active.length} live</span></div>{loading ? <div className="mt-10 grid gap-6 md:grid-cols-2"><div className="loading-card" /><div className="loading-card" /></div> : active.length === 0 ? <div className="empty-state mt-10"><Sparkles className="mx-auto size-10 text-brand-yellow" /><h3 className="mt-4 text-xl font-black">Today’s giveaway is on the way</h3><p className="mt-2 text-muted-foreground">Follow Telugu Toon World on Instagram so you never miss the next drop.</p></div> : <div className="mt-10 grid gap-6 md:grid-cols-2">{active.map((giveaway, index) => <GiveawayCard key={giveaway.id ?? index} giveaway={giveaway} featured={giveaway.id === featured?.id} />)}</div>}</section>{!loading && <section id="upcoming" className="upcoming-band mx-auto max-w-7xl px-5 pb-20 lg:px-8 lg:pb-28"><div className="upcoming-heading"><div><div className="eyebrow"><CalendarDays className="size-4" /> Coming soon</div><h2 className="section-title mt-4">Upcoming giveaways.</h2><p className="mt-3 max-w-xl text-muted-foreground">Get ready for the next drop. Entries open automatically when the scheduled start time arrives.</p></div><span className="rounded-full bg-brand-yellow/25 px-4 py-2 text-sm font-black text-foreground">{upcoming.length} scheduled</span></div>{upcoming.length === 0 ? <div className="empty-state mt-10"><CalendarDays className="mx-auto size-10 text-brand-teal" /><h3 className="mt-4 text-xl font-black">No upcoming giveaways yet</h3><p className="mt-2 text-muted-foreground">Check back soon for the next Telugu Toon World drop.</p></div> : <div className="mt-10 grid gap-6 md:grid-cols-2">{upcoming.map((giveaway, index) => <UpcomingCard key={giveaway.id ?? index} giveaway={giveaway} now={now} />)}</div>}</section>}<section className="past-band"><div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24"><div className="flex flex-wrap items-end justify-between gap-5"><div><div className="eyebrow"><Trophy className="size-4" /> Past winners</div><h2 className="section-title mt-4">Good vibes, <span className="text-brand-coral">great winners.</span></h2></div><Button variant="outline" className="text-primary-foreground border-primary-foreground/40 bg-transparent hover:bg-primary-foreground/10 hover:text-primary-foreground" asChild><Link to="/past-winners">View the hall of fame <ChevronRight /></Link></Button></div>{completed.length === 0 ? <p className="mt-8 text-muted-foreground">Your first winner leaderboard will live here after the draw.</p> : <div className="mt-9 grid gap-6 lg:grid-cols-2">{completed.slice(0, 2).map((giveaway) => <WinnerPreview key={giveaway.id ?? giveaway.title ?? "winner"} giveaway={giveaway} winners={winners.filter((winner) => winner.giveaway_id === giveaway.id)} />)}</div>}</div></section><footer className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-8 text-sm text-muted-foreground lg:px-8"><p>Made with joy for the Toon World family.</p><a href="https://www.instagram.com/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 font-bold text-foreground hover:text-brand-coral"><Instagram className="size-4" /> @telugutoonworld</a></footer>{showSplash && <IntroSplash onDone={() => setShowSplash(false)} />}</main>;
}
...
function GiveawayCard({ giveaway, featured }: { giveaway: Giveaway; featured: boolean }) {
  ...
  return <article className={featured ? "giveaway-card giveaway-card-featured" : "giveaway-card"}><div className="relative overflow-hidden rounded-[1.25rem]"><img src={giveaway.image_url || heroImage} alt={giveaway.title ?? "Giveaway prize"} className="aspect-[16/9] w-full object-cover" width={720} height={405} loading="lazy" /><span className="absolute left-4 top-4 rounded-full bg-background/90 px-3 py-1 text-xs font-black uppercase tracking-[0.12em]">{featured ? "Featured" : "Open"}</span></div><div className="pt-5"><div className="flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-[0.1em] text-muted-foreground"><span className="inline-flex items-center gap-1.5"><Clock3 className="size-3.5 text-brand-coral" /> {days} days left</span><span>{giveaway.winner_limit} winner{giveaway.winner_limit === 1 ? "" : "s"}</span></div><h3 className="mt-3 text-2xl font-black tracking-tight">{giveaway.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-muted-foreground">{giveaway.description}</p><SocialLinks giveaway={giveaway} /><div className="mt-5 flex items-center justify-between gap-4"><span className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground"><Users className="size-4 text-brand-teal" /> {giveaway.participant_count ?? 0} joined</span><Button asChild><Link to="/giveaways/$id" params={{ id: giveaway.id ?? "" }}>Enter now <ArrowUpRight /></Link></Button></div></div></article>;
}

function UpcomingCard({ giveaway, now }: { giveaway: Giveaway; now: number | null }) {
  const starts = new Date(giveaway.start_date ?? Date.now());
  return <article className="upcoming-card"><div className="relative overflow-hidden rounded-[1.25rem]"><img src={giveaway.image_url || heroImage} alt={giveaway.title ?? "Upcoming giveaway"} className="aspect-[16/9] w-full object-cover" width={720} height={405} loading="lazy" /><span className="absolute left-4 top-4 rounded-full bg-background/90 px-3 py-1 text-xs font-black uppercase tracking-[0.12em]">Coming soon</span></div><div className="pt-5"><div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.1em] text-brand-teal"><CalendarDays className="size-3.5" /> Opens {starts.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div><h3 className="mt-3 text-2xl font-black tracking-tight">{giveaway.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-muted-foreground">{giveaway.description}</p><SocialLinks giveaway={giveaway} /><div className="mt-5 flex flex-wrap items-end justify-between gap-4"><span className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground"><Trophy className="size-4 text-brand-yellow" /> {giveaway.winner_limit} winner{giveaway.winner_limit === 1 ? "" : "s"}</span><div className="countdown-wrap"><span className="countdown-label">Starts in</span><time className="countdown" dateTime={starts.toISOString()} aria-label={`Giveaway starts in ${formatCountdown(starts.getTime(), now)}`}>{formatCountdown(starts.getTime(), now)}</time></div></div></div></article>;
}

function SocialLinks({ giveaway }: { giveaway: Giveaway }) {
  const links = [
    giveaway.instagram_url ? { href: giveaway.instagram_url, label: "Instagram", icon: Instagram } : null,
    giveaway.telegram_url ? { href: giveaway.telegram_url, label: "Telegram", icon: Send } : null,
    giveaway.youtube_url ? { href: giveaway.youtube_url, label: "YouTube", icon: Youtube } : null,
    giveaway.facebook_url ? { href: giveaway.facebook_url, label: "Facebook", icon: Facebook } : null,
  ].filter((link): link is { href: string; label: string; icon: typeof Instagram } => Boolean(link));
  if (links.length === 0) return null;
  return <div className="social-links" aria-label="Giveaway community links">{links.map(({ href, label, icon: Icon }) => <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={label} title={label}><Icon className="size-4" /><span>{label}</span></a>)}</div>;
}

function formatCountdown(targetTime: number, now: number | null) {
  if (now === null) return "Calculating…";
  const remaining = Math.max(0, targetTime - now);
  const days = Math.floor(remaining / 86400000);
  const hours = Math.floor((remaining % 86400000) / 3600000);
  const minutes = Math.floor((remaining % 3600000) / 60000);
  const seconds = Math.floor((remaining % 60000) / 1000);
  return days > 0 ? `${days}d ${hours}h ${minutes}m` : `${String(hours).padStart(2, "0")}h ${String(minutes).padStart(2, "0")}m ${String(seconds).padStart(2, "0")}s`;
}
