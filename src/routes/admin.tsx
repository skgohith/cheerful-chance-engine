...
function AdminStudio() {
  ...
  if (!session) return <main className="site-shell flex min-h-screen items-center justify-center px-5"><div className="auth-panel"><Link to="/" className="brand-lockup"><span className="brand-mark">TT</span><span>Telugu Toon World</span></Link><div className="mt-10"><div className="eyebrow"><ShieldCheck className="size-4" /> Private studio</div><h1 className="mt-4 text-3xl font-black tracking-tight">Welcome back.</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Sign in with an admin account to post today’s giveaway, manage entries, and draw winners.</p></div>{message && <p className="status-error mt-6">{message}</p>}<form className="mt-7 space-y-4" onSubmit={signIn}><label className="field-label">Email<Input required type="email" value={loginForm.email} onChange={(event) => setLoginForm({ ...loginForm, email: event.target.value })} /></label><label className="field-label">Password<Input required type="password" value={loginForm.password} onChange={(event) => setLoginForm({ ...loginForm, password: event.target.value })} /></label><Button className="w-full" size="lg" disabled={busy}>{busy ? "Signing in…" : <><LogIn /> Sign in</>}</Button></form><p className="mt-6 text-center text-xs text-muted-foreground">Admin access is restricted to accounts with the admin role.</p></div></main>;
