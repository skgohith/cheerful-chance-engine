...
function AdminStudio() {
  ...
  async function saveParticipantInstagramLink(participant: Participant) { const instagramLink = linkDrafts[participant.id] ?? ""; setSavingLinkId(participant.id); setMessage(null); try { const updated = await updateParticipantInstagramLink({ data: { participantId: participant.id, giveawayId: participant.giveaway_id, instagramLink } }); setParticipants((current) => current.map((row) => row.id === updated.id ? updated : row)); setLinkDrafts((current) => ({ ...current, [updated.id]: updated.instagram_link })); setMessage("Instagram link updated."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to update Instagram link"); } finally { setSavingLinkId(null); } }
