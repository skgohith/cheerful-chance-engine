...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Manage Telugu Toon World giveaways, entries, and winner draws.",
      },
...
function AdminStudio() {
  ...
  async function remove(id: string) {
    if (
      !window.confirm(
        "Delete this giveaway and its entries? Any selected winners will be preserved in the hall of fame.",
      )
    )
      return;
    setBusy(true);
        ...
        <div className="auth-panel">
          ...
          {showPasswordReset ? (
            <>
              <div className="mt-10">
                <div className="eyebrow">
                  <KeyRound className="size-4" /> Account recovery
                </div>
                <h1 className="mt-4 text-3xl font-black tracking-tight">Reset your password.</h1>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  We’ll email a secure reset link to the authorized admin account.
                </p>
              </div>
              {message && <p className="status-error mt-6">{message}</p>}
              {resetSent && (
                <p className="status-success mt-6">
                  If the email is registered, a reset link is on its way. Check your inbox.
                </p>
              )}
            ...
            <>
              <div className="mt-10">
                <div className="eyebrow">
                  <ShieldCheck className="size-4" /> Private studio
                </div>
                <h1 className="mt-4 text-3xl font-black tracking-tight">Welcome back.</h1>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  Sign in with the authorized admin account to post today’s giveaway, manage
                  entries, and draw winners.
                </p>
              </div>
              {message && <p className="status-error mt-6">{message}</p>}
              ...
              <p className="mt-2 text-center text-xs text-muted-foreground">
                This private studio is limited to the authorized admin account. New admin signups
                are disabled.
              </p>
            </>
          )}
    ...
    <main className="site-shell min-h-screen pb-16">
      <header className="site-header mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
        <Link to="/" className="brand-lockup">
          <span className="brand-mark">TT</span>
          <span>
            Telugu Toon World <span className="admin-tag">Studio</span>
          </span>
        </Link>
      ...
      <div className="mx-auto max-w-7xl px-5 pt-10 lg:px-8 lg:pt-16">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            <div className="eyebrow">
              <ShieldCheck className="size-4" /> Admin studio
            </div>
            <h1 className="section-title mt-4">Make the magic happen.</h1>
            <p className="mt-3 text-muted-foreground">
              Post today’s giveaway, watch entries roll in, and lock in fair winners.
            </p>
          </div>
        ...
        {editing && (
          <form className="admin-form mt-8" onSubmit={save}>
            <div className="flex items-center justify-between">
              <div>
                <p className="section-kicker">Giveaway details</p>
                <h2 className="mt-1 text-xl font-black">
                  {selectedId ? "Edit giveaway" : "Post a giveaway"}
                </h2>
              </div>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <label className="field-label">
                Image URL <span className="optional-mark">Optional</span>
                <Input
                  type="url"
                  maxLength={500}
                  value={form.imageUrl}
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Add the official channels participants should visit for this giveaway.
                </p>
              </div>
              <label className="field-label">
                Instagram <span className="optional-mark">Optional</span>
                <Input
                  type="url"
                  maxLength={500}
              ...
              <label className="field-label">
                Telegram <span className="optional-mark">Optional</span>
                <Input
                  type="url"
                  maxLength={500}
                  value={form.telegramUrl}
                  onChange={(event) => setForm({ ...form, telegramUrl: event.target.value })}
                  placeholder="https://t.me/…"
                />
              </label>
              <label className="field-label">
                YouTube <span className="optional-mark">Optional</span>
                <Input
                  type="url"
                  maxLength={500}
              ...
              <label className="field-label">
                Facebook <span className="optional-mark">Optional</span>
                <Input
                  type="url"
                  maxLength={500}
              ...
              <label className="field-label">
                ...
                <Input
                  ...
                  onChange={(event) => setForm({ ...form, winnerLimit: event.target.value })}
                />
              </label>
            </div>
            <Button type="submit" className="mt-6" disabled={busy}>
              {busy ? "Saving…" : "Post giveaway"}
            </Button>
          </form>
        )}
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[.9fr_1.1fr]">
          <section>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-black">Your giveaways</h2>
              <Button
                variant="ghost"
                size="icon"
                onClick={() => void refreshGiveaways()}
                aria-label="Refresh giveaways"
              >
                <RefreshCw />
              </Button>
            </div>
            <div className="space-y-3">
              {giveaways.length === 0 && (
                <div className="empty-state">
                  <WandSparkles className="mx-auto size-8 text-brand-yellow" />
                  <p className="mt-3 text-sm text-muted-foreground">Create your first giveaway.</p>
                </div>
              )}
              {giveaways.map((giveaway) => {
                const locked =
                  giveaway.status === "completed" || giveaway.status === "data_cleared";
                return (
                  <div
                    key={giveaway.id}
                    className={
                      selectedId === giveaway.id
                        ? "admin-list-item admin-list-item-selected"
                        : "admin-list-item"
                    }
                  >
                    <button
                      type="button"
                      onClick={() => setSelectedId(giveaway.id ?? null)}
                      className="min-w-0 flex-1 text-left"
                    >
                      <strong className="block truncate">{giveaway.title}</strong>
                      <small>{giveaway.status} · select to view entries</small>
                    </button>
                    <span className="flex shrink-0 gap-1">
                      {!locked && (
                        <Button
                          ...
                          aria-label={`Edit ${giveaway.title}`}
                          title="Edit"
                        >
                          <Pencil />
                        </Button>
                      ...
                      {locked && (
                        <span
                          className="inline-flex items-center px-2 text-xs font-bold text-muted-foreground"
                          title="Editing is disabled after winners are selected"
                        >
                          <LockKeyhole className="mr-1 size-3.5" /> Locked
                        </span>
                      )}
                      <Button
                        type="button"
                        ...
                        onClick={() => void remove(giveaway.id ?? "")}
                        aria-label={`Delete ${giveaway.title}`}
            ...
            {selected ? (
              <div className="admin-detail">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="section-kicker">Selected giveaway</p>
                    <h2 className="mt-2 text-2xl font-black">{selected.title}</h2>
                    <p className="mt-1 text-sm text-muted-foreground">{selected.description}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {selected.status === "active" && (
                      <Button onClick={() => void draw(selected.id ?? "")} disabled={busy}>
                        <Trophy /> Pick winners
                      </Button>
                    )}
                    {selected.status === "completed" && (
                      <Button
                        variant="outline"
                        onClick={() => void clearEntries(selected.id ?? "")}
                        disabled={busy}
                      >
                        <Trash2 /> Clear entries
                      </Button>
                    )}
                  </div>
                </div>
                <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  <Stat icon={<Users />} label="Entries" value={String(participants.length)} />
                  <Stat
                    icon={<Trophy />}
                    label="Winner spots"
                    value={String(selected.winner_limit)}
                  />
                  <Stat icon={<ShieldCheck />} label="Status" value={selected.status ?? "active"} />
                </div>
                <div className="mt-8">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h3 className="font-black">Participants ({participants.length})</h3>
                      <p className="mt-1 text-sm text-muted-foreground">
                        Correct an entrant’s Instagram profile link when needed.
                      </p>
                    </div>
                    <Button
                      variant="outline"
                    ...
                    <table className="data-table">
                      ...
                      <tbody>
                        {participants.map((participant) => {
                          const draft = linkDrafts[participant.id] ?? participant.instagram_link;
                          const isSaving = savingLinkId === participant.id;
                          const validDraft = isHttpUrl(draft);
                          return (
                            <tr key={participant.id}>
                              <td>{participant.full_name}</td>
                              <td>{participant.instagram_username}</td>
                              <td>
                                <div className="flex min-w-[20rem] items-center gap-2">
                                  <Input
                                    aria-label={`Instagram link for ${participant.full_name}`}
                                    type="url"
                                    value={draft}
                                    onChange={(event) =>
                                  ...
                                  <Button
                                    ...
                                    size="icon"
                                    variant="outline"
                                    aria-label={`Save Instagram link for ${participant.full_name}`}
                                    title="Save Instagram link"
                                    disabled={
                                      isSaving ||
                                      draft === participant.instagram_link ||
                                      !validDraft
                                    }
                                    onClick={() => void saveParticipantInstagramLink(participant)}
                                  >
                                    <Check />
                                  </Button>
                                  {validDraft && (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noreferrer"
                                      aria-label={`Open Instagram link for ${participant.full_name}`}
                              ...
                              <td>
                                {participant.created_at
                                  ? new Date(participant.created_at).toLocaleDateString("en-IN")
                                  : "—"}
                              </td>
                            </tr>
                          );
                ...
                {winners.length > 0 && (
                  <div className="mt-8">
                    <h3 className="font-black">Winner snapshot</h3>
                    <div className="mt-3 space-y-2">
                      {winners.map((winner) => (
                        <div className="winner-row" key={winner.id}>
                          <span className="rank-badge">{winner.rank}</span>
                          <span className="flex-1 font-bold">{winner.full_name}</span>
              ...
              <div className="empty-state h-full">
                <Users className="mx-auto size-10 text-brand-teal" />
                <h2 className="mt-4 text-xl font-black">Select a giveaway</h2>
                <p className="mt-2 text-muted-foreground">
                  Choose a giveaway to view entries and manage its draw.
                </p>
              </div>
            )}
          </section>
