...
import { ArrowLeft, Check, Download, ExternalLink, KeyRound, LockKeyhole, LogIn, LogOut, Mail, Pencil, Plus, RefreshCw, ShieldCheck, Trash2, Trophy, Users, WandSparkles } from "lucide-react";
...
import { adminClearData, adminDeleteGiveaway, adminListGiveaways, adminListParticipants, adminListWinners, adminPickWinners, adminSaveGiveaway, adminUpdateParticipantInstagramLink } from "@/lib/giveaway.functions";
...
function AdminStudio() {
  ...
  useEffect(() => { if (!selectedId) return; void Promise.all([loadParticipants({ data: { giveawayId: selectedId } }), loadWinners({ data: { giveawayId: selectedId } })]).then(([rows, winnerRows]) => { setParticipants(rows); setLinkDrafts(Object.fromEntries(rows.map((row) => [row.id, row.instagram_link]))); setWinners(winnerRows); }); }, [selectedId, loadParticipants, loadWinners]);
  ...
  async function sendPasswordReset(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setMessage(null); setResetSent(false);
    const email = loginForm.email.trim().toLowerCase();
    if (email !== "germanbro40@gmail.com") { setMessage("Enter the authorized admin email to receive a reset link."); setBusy(false); return; }
  ...
  async function remove(id: string) { if (!window.confirm("Delete this giveaway and all of its entries? This cannot be undone.")) return; setBusy(true); setMessage(null); try { await deleteGiveaway({ data: { giveawayId: id } }); if (selectedId === id) { setSelectedId(null); setParticipants([]); setWinners([]); } await refreshGiveaways(); setMessage("Giveaway and its entries deleted."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to delete giveaway"); } finally { setBusy(false); } }
  ...
  async function saveParticipantInstagramLink(participant: Participant) { const instagramLink = linkDrafts[participant.id] ?? ""; if (!selectedId) return; setSavingLinkId(participant.id); setMessage(null); try { const updated = await updateParticipantInstagramLink({ data: { participantId: participant.id, giveawayId: selectedId, instagramLink } }); setParticipants((current) => current.map((row) => row.id === updated.id ? updated : row)); setLinkDrafts((current) => ({ ...current, [updated.id]: updated.instagram_link })); setMessage("Instagram link updated."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to update Instagram link"); } finally { setSavingLinkId(null); } }
  ...
  if (!session) return <main className="site-shell flex min-h-screen items-center justify-center px-5"><div className="auth-panel"><Link to="/" className="brand-lockup"><span className="brand-mark">TT</span><span>Telugu Toon World</span></Link>{showPasswordReset ? <><div className="mt-10"><div className="eyebrow"><KeyRound className="size-4" /> Account recovery</div><h1 className="mt-4 text-3xl font-black tracking-tight">Reset your password.</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">We’ll email a secure reset link to the authorized admin account.</p></div>{message && <p className="status-error mt-6">{message}</p>}{resetSent && <p className="status-success mt-6">If the email is registered, a reset link is on its way. Check your inbox.</p>}<form className="mt-7 space-y-4" onSubmit={sendPasswordReset}><label className="field-label">Admin email<Input required type="email" autoComplete="email" value={loginForm.email} onChange={(event) => setLoginForm({ ...loginForm, email: event.target.value })} /></label><Button className="w-full" size="lg" disabled={busy}>{busy ? "Sending…" : <><Mail /> Send reset link</>}</Button></form><Button type="button" variant="ghost" className="mt-4 w-full" onClick={() => { setShowPasswordReset(false); setMessage(null); setResetSent(false); }}><ArrowLeft /> Back to sign in</Button></> : <><div className="mt-10"><div className="eyebrow"><ShieldCheck className="size-4" /> Private studio</div><h1 className="mt-4 text-3xl font-black tracking-tight">Welcome back.</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Sign in with the authorized admin account to post today’s giveaway, manage entries, and draw winners.</p></div>{message && <p className="status-error mt-6">{message}</p>}<form className="mt-7 space-y-4" onSubmit={signIn}><label className="field-label">Admin email<Input required type="email" autoComplete="username" value={loginForm.email} onChange={(event) => setLoginForm({ ...loginForm, email: event.target.value })} /></label><label className="field-label">Password<Input required type="password" autoComplete="current-password" value={loginForm.password} onChange={(event) => setLoginForm({ ...loginForm, password: event.target.value })} /></label><Button className="w-full" size="lg" disabled={busy}>{busy ? "Signing in…" : <><LogIn /> Sign in</>}</Button></form><Button type="button" variant="link" className="mt-4 w-full" onClick={() => { setShowPasswordReset(true); setMessage(null); }}>Forgot admin password?</Button><p className="mt-2 text-center text-xs text-muted-foreground">This private studio is limited to the authorized admin account. New admin signups are disabled.</p></>}</div></main>;

  const selected = giveaways.find((giveaway) => giveaway.id === selectedId);
  return <div className="mt-10 grid gap-8 lg:grid-cols-[.9fr_1.1fr]"><section><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-black">Your giveaways</h2><Button variant="ghost" size="icon" onClick={() => void refreshGiveaways()} aria-label="Refresh giveaways"><RefreshCw /></Button></div><div className="space-y-3">{giveaways.length === 0 && <div className="empty-state"><WandSparkles className="mx-auto size-8 text-brand-yellow" /><p className="mt-3 text-sm text-muted-foreground">Create your first giveaway.</p></div>}{giveaways.map((giveaway) => { const locked = giveaway.status === "completed" || giveaway.status === "data_cleared"; return <div key={giveaway.id} className={selectedId === giveaway.id ? "admin-list-item admin-list-item-selected" : "admin-list-item"}><button type="button" onClick={() => setSelectedId(giveaway.id ?? null)} className="min-w-0 flex-1 text-left"><strong className="block truncate">{giveaway.title}</strong><small>{giveaway.status} · select to view entries</small></button><span className="flex shrink-0 gap-1">{!locked && <Button type="button" variant="ghost" size="icon" className="icon-action" onClick={() => startEdit(giveaway)} aria-label={`Edit ${giveaway.title}`} title="Edit"><Pencil /></Button>}{locked ? <span className="inline-flex items-center px-2 text-xs font-bold text-muted-foreground" title="Completed giveaway data is preserved"><LockKeyhole className="mr-1 size-3.5" /> Locked</span> : <Button type="button" variant="ghost" size="icon" className="icon-action icon-action-danger" onClick={() => void remove(giveaway.id ?? "")} aria-label={`Delete ${giveaway.title}`} title="Delete"><Trash2 /></Button>}</span></div>; })}</div></section><section>{selected ? <div className="admin-detail"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="section-kicker">Selected giveaway</p><h2 className="mt-2 text-2xl font-black">{selected.title}</h2><p className="mt-1 text-sm text-muted-foreground">{selected.description}</p></div><div className="flex flex-wrap gap-2">{selected.status === "active" && <Button onClick={() => void draw(selected.id ?? "")} disabled={busy}><Trophy /> Pick winners</Button>}{selected.status === "completed" && <Button variant="outline" onClick={() => void clearEntries(selected.id ?? "")} disabled={busy}><Trash2 /> Clear entries</Button>}</div></div><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3"><Stat icon={<Users />} label="Entries" value={String(participants.length)} /><Stat icon={<Trophy />} label="Winner spots" value={String(selected.winner_limit)} /><Stat icon={<ShieldCheck />} label="Status" value={selected.status ?? "active"} /></div><div className="mt-8"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">Participants ({participants.length})</h3><p className="mt-1 text-sm text-muted-foreground">Correct an entrant’s Instagram profile link when needed.</p></div><Button variant="outline" size="sm" onClick={exportCsv} disabled={participants.length === 0}><Download /> Export CSV</Button></div><div className="mt-4 overflow-x-auto"><table className="data-table"><thead><tr><th>Name</th><th>Instagram</th><th>Instagram link</th><th>Joined</th></tr></thead><tbody>{participants.map((participant) => { const draft = linkDrafts[participant.id] ?? participant.instagram_link; const isSaving = savingLinkId === participant.id; return <tr key={participant.id}><td>{participant.full_name}</td><td>{participant.instagram_username}</td><td><div className="flex min-w-[20rem] items-center gap-2"><Input aria-label={`Instagram link for ${participant.full_name}`} type="url" value={draft} onChange={(event) => setLinkDrafts((current) => ({ ...current, [participant.id]: event.target.value }))} /><Button type="button" size="icon" variant="outline" aria-label={`Save Instagram link for ${participant.full_name}`} title="Save Instagram link" disabled={isSaving || draft === participant.instagram_link} onClick={() => void saveParticipantInstagramLink(participant)}><Check /></Button>{draft && <a className="text-brand-coral" href={draft} target="_blank" rel="noreferrer" aria-label={`Open Instagram link for ${participant.full_name}`} title="Open Instagram link"><ExternalLink className="size-4" /></a>}</div></td><td>{participant.created_at ? new Date(participant.created_at).toLocaleDateString("en-IN") : "—"}</td></tr>; })}</tbody></table>{participants.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">No participants loaded yet.</p>}</div></div>{winners.length > 0 && <div className="mt-8"><h3 className="font-black">Winner snapshot</h3><div className="mt-3 space-y-2">{winners.map((winner) => <div className="winner-row" key={winner.id}><span className="rank-badge">{winner.rank}</span><span className="flex-1 font-bold">{winner.full_name}</span><span className="text-sm text-muted-foreground">{winner.instagram_username}</span></div>)}</div></div>}</div> : <div className="empty-state h-full"><Users className="mx-auto size-10 text-brand-teal" /><h2 className="mt-4 text-xl font-black">Select a giveaway</h2><p className="mt-2 text-muted-foreground">Choose a giveaway to view entries and manage its draw.</p></div>}</section></div></div></main>;
}
